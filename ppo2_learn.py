...
def main(total_timesteps,alpha,gamma):
    ...
    logger.configure(dir="./logs/ppo2", format_strs=['stdout', 'log', 'csv', 'tensorboard'])

    env = make_aai_env("AnimalAI/AnimalAI", 1, arenas_configurations)
    act = ppo2.learn(env=env, network="cnn", total_timesteps=total_timesteps, lr=alpha, gamma=gamma,log_interval=1)

    # Guardar el modelo entrenado
    print("Guardando modelo en ppo2_model.pkl")
    act.save("./logs/ppo2/ppo2_model.pkl")

if __name__ == "__main__":
    # total_timesteps,alpha,gamma
    argv = [str(arg) for arg in sys.argv[1:]]
    main(int(argv[0]),float(argv[1]),float(argv[2]))
    # python -i .\ppo2_learn.py 100000 0.001 0.95